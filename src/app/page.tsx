...
export default async function Dashboard() {
  ...
  const followerLabels = followerMetrics.map((m) => dayjs(m.timestamp).format('MM-DD'));
  const followerData = followerMetrics.map((m) => m.followers ?? 0);
  ...
  const viewLabels = viewMetrics.map((m) => dayjs(m.timestamp).format('MM-DD'));
  const viewData = viewMetrics.map((m) => m.views ?? 0);

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="p-4 rounded bg-gray-100 dark:bg-gray-700">{t(locale, 'accounts')}: {accountsCount}</div>
        <div className="p-4 rounded bg-gray-100 dark:bg-gray-700">7d Followers: {followerData.slice(-7).reduce((a,b)=>a+b,0)}</div>
        <div className="p-4 rounded bg-gray-100 dark:bg-gray-700">7d Views: {viewData.slice(-7).reduce((a,b)=>a+b,0)}</div>
